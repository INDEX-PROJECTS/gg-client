...
const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<'ol'>>(
  ({ className, ...props }, ref) => (
    <ol
      ref={ref}
      className={cn(
        't2 max-[450px]:t-xs mb-[42px] flex flex-wrap items-center gap-x-[8px] gap-y-[4px] break-words max-[450px]:mb-[16px] max-[450px]:gap-[4px] max-[450px]:gap-y-0',
        className,
      )}
      {...props}
    />
...
const BreadcrumbPage = React.forwardRef<HTMLSpanElement, React.ComponentPropsWithoutRef<'span'>>(
  ({ className, ...props }, ref) => (
    <span
      ...
      aria-disabled="true"
      aria-current="page"
      className={cn('text-backdrop', className)}
      {...props}
    />
  ),
);
